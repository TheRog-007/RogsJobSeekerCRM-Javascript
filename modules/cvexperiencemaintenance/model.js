...
export const dteCVE_YearStarted = document.getElementById("dteCVE_YearStarted");
export const lblCVE_YearStarted = document.getElementById("lblCVE_YearStarted");
export const dteCVE_YearEnded = document.getElementById("dteCVE_YearEnded");
export const lblCVE_YearEnded = document.getElementById("lblCVE_YearEnded");
export const txtCVE_Where = document.getElementById("txtCVE_Where");
  ...
  function funcCreateItems() {
    ...
    qryTemp.onsuccess = (event) => {
      ...
      if (curTemp) {
        ...
        elTemp.innerText = curTemp.value.CVE_Role;
...
*/
  ...
  const objTemp = dbRequest.objectStore(modSchema.constCV_Experience);

  if (blnNew) {
    dbUpdate = objTemp.add({
      CVE_YearStarted: dteCVE_YearStarted.value,
    ...
    dbQuery.onsuccess = () => {
      ...
      //edit
      dbData.CVE_YearStarted = dteCVE_YearStarted.value;
...
// let objTemp = modSchema.funcGetFirstRecord(constCV_Experience);
...
*/
  ...
  if (blnNew) {
    ...
    dteCVE_YearStarted.value = objTable.aryFields[1].fieldValue;
...
*/
  ...
  if (blnNew && dteCVE_YearEnded.value.length !== 0) {
    modMessageBox.funcMessageBox(
      "Record Not Saved, Save?",
      modMessageBox.objIcons.exclamation,
      modMessageBox.objButtons.yes,
...
export function funcInitSchema() {
  //get schema
  objTable = modSchema.funcGetSchema(modSchema.constCV_Experience);
}
 ...
 */
  ...
  // //get first record
  // objData = modSchema.funcGetFirstRecord(modSchema.constCV_Experience);
}
