...
export const txtCVPP_Personality = document.getElementById(
  "txtCVPP_Personality"
);
export const lblCVPP_Personality = document.getElementById(
  "lblCVPP_Personality"
);
  ...
  function funcCreateItems() {
    ...
    qryTemp.onsuccess = (event) => {
      ...
      if (curTemp) {
        ...
        elTemp.innerText = curTemp.value.CVPP_Personality;
...
*/
  ...
  const objTemp = dbRequest.objectStore(modSchema.constCV_Personality);

  if (blnNew) {
    dbUpdate = objTemp.add({ CVPP_Personality: txtCVPP_Personality.value });
    ...
    dbQuery.onsuccess = () => {
      ...
      //edit
      dbData.CVPP_Personality = txtCVPP_Personality.value;
...
// let objTemp = modSchema.funcGetFirstRecord(constCV_Personality);
...
*/
  ...
  if (blnNew) {
    ...
    txtCVPP_Personality.value = objTable.aryFields[1].fieldValue;
  }
}
...
*/
  ...
  txtCVPP_Personality.value = "";
  txtCVPP_Personality.focus();
};
...
*/
  ...
  if (blnNew && txtCVPP_Personality.value.length !== 0) {
    modMessageBox.funcMessageBox(
      "Record Not Saved, Save?",
      modMessageBox.objIcons.exclamation,
      modMessageBox.objButtons.yes,
...
export function funcInitSchema() {
  //get schema
  objTable = modSchema.funcGetSchema(modSchema.constCV_Personality);
}
 ...
 */
  ...
  // //get first record
  // objData = modSchema.funcGetFirstRecord(modSchema.constCV_Personality);
}
export function funcInitHandlers() {
  ...
  */
  ...
  txtCVPP_Personality.addEventListener("keydown", functextboxKeyDown);
