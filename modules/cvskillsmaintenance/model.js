...
export const txtCVS_Details = document.getElementById("txtCVS_Details");
export const lblCVS_Details = document.getElementById("lblCVS_Details");
export const txtCVS_Name = document.getElementById("txtCVS_Name");
export const lblCVS_Name = document.getElementById("lblCVS_Name");
  ...
  function funcCreateItems() {
    ...
    qryTemp.onsuccess = (event) => {
      ...
      if (curTemp) {
        ...
        elTemp.innerText = curTemp.value.CVS_Name;
...
*/
  ...
  const objTemp = dbRequest.objectStore(modSchema.constCV_Skills);

  if (blnNew) {
    dbUpdate = objTemp.add({
      CVS_Name: txtCVS_Name.value,
      CVS_Details: txtCVS_Details.value,
    });
    ...
    dbQuery.onsuccess = () => {
      ...
      //edit
      dbData.CVS_Details = txtCVS_Details.value;
...
// let objTemp = modSchema.funcGetFirstRecord(constCV_Skills);
...
*/
  ...
  if (blnNew) {
    ...
    txtCVS_Name.value = objTable.aryFields[1].fieldValue;
    txtCVS_Details.value = objTable.aryFields[2].fieldValue;
  }
}
...
*/
  ...
  txtCVS_Details.value = "";
  txtCVS_Name.value = "";
  txtCVS_Details.focus();
};
...
*/
  ...
  if (blnNew && txtCVS_Details.value.length !== 0) {
    modMessageBox.funcMessageBox(
      "Record Not Saved, Save?",
      modMessageBox.objIcons.exclamation,
      modMessageBox.objButtons.yes,
...
export function funcInitSchema() {
  //get schema
  objTable = modSchema.funcGetSchema(modSchema.constCV_Skills);
}
 ...
 */
  ...
  // //get first record
  // objData = modSchema.funcGetFirstRecord(modSchema.constCV_Skills);
}
export function funcInitHandlers() {
  ...
  */
  ...
  txtCVS_Details.addEventListener("keydown", functextboxKeyDown);
  txtCVS_Name.addEventListener("keydown", functextboxKeyDown);
