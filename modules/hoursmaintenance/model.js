...
export const txtHRS_Hours = document.getElementById("txtHRS_Hours");
export const lblHRS_Hours = document.getElementById("lblHRS_Hours");
  ...
  function funcCreateItems() {
    ...
    qryTemp.onsuccess = (event) => {
      ...
      if (curTemp) {
        ...
        elTemp.innerText = curTemp.value.HRS_Hours;
...
*/
  ...
  const objTemp = dbRequest.objectStore(modSchema.constSeekers_Hours);

  if (blnNew) {
    dbUpdate = objTemp.add({ HRS_Hours: txtHRS_Hours.value });
    ...
    dbQuery.onsuccess = () => {
      ...
      //edit
      dbData.HRS_Hours = txtHRS_Hours.value;
...
// let objTemp = modSchema.funcGetFirstRecord(constSeekers_Hours);
...
*/
  ...
  if (blnNew) {
    ...
    txtHRS_Hours.value = objTable.aryFields[1].fieldValue;
  }
}
...
*/
  ...
  if (blnNew && txtHRS_Hours.value.length !== 0) {
    modMessageBox.funcMessageBox(
      "Record Not Saved, Save?",
      modMessageBox.objIcons.exclamation,
      modMessageBox.objButtons.yes,
...
export function funcInitSchema() {
  //get schema
  objTable = modSchema.funcGetSchema(modSchema.constSeekers_Hours);
}
...
export function funcInitHandlers() {
  ...
  */
  ...
  txtHRS_Hours.addEventListener("keydown", functextboxKeyDown);
