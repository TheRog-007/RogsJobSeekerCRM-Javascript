...
export const txtSTA_Status = document.getElementById("txtSTA_Status");
export const lblSTA_Status = document.getElementById("lblSTA_Status");
  ...
  */
  ...
  function funcCreateItems() {
    ...
    const trnTemp = dbJobSeekerCRM.transaction(
      modSchema.constSeekers_Status,
      "readonly"
    );
    const objTemp = trnTemp.objectStore(modSchema.constSeekers_Status);
    ...
    qryTemp.onsuccess = (event) => {
      ...
      if (curTemp) {
        ...
        elTemp.innerText = curTemp.value.STA_Status;
...
*/
  ...
  const objTemp = dbRequest.objectStore(modSchema.constSeekers_Status);

  if (blnNew) {
    dbUpdate = objTemp.add({ STA_Status: txtSTA_Status.value });
    ...
    dbQuery.onsuccess = () => {
      ...
      //edit
      dbData.STA_Status = txtSTA_Status.value;
...
*/
  ...
  if (blnNew) {
    ...
    txtSTA_Status.value = objTable.aryFields[1].fieldValue;
  }
}
...
*/
  ...
  if (blnNew && txtSTA_Status.value.length !== 0) {
    modMessageBox.funcMessageBox(
      "Record Not Saved, Save?",
      modMessageBox.objIcons.exclamation,
      modMessageBox.objButtons.yes,
...
export function funcInitHandlers() {
  ...
  */
  ...
  txtSTA_Status.addEventListener("keydown", functextboxKeyDown);
